...
describe('ElfaSDK', () => {
  ...
  beforeEach(() => {
    ...
    (ElfaV2Client as jest.MockedClass<typeof ElfaV2Client>).mockImplementation(() => mockElfaClient);
    (TwitterClient as jest.MockedClass<typeof TwitterClient>).mockImplementation(() => mockTwitterClient);
    (ResponseEnhancer as jest.MockedClass<typeof ResponseEnhancer>).mockImplementation(() => mockEnhancer);
  });
  ...
  describe('basic API methods', () => {
    ...
    it('should call ping', async () => {
      const mockResponse = { success: true as const, data: { message: 'pong' } };
  ...
  describe('testConnection', () => {
    ...
    it('should handle Elfa connection failure', async () => {
      ...
      mockElfaClient.testConnection.mockRejectedValue(new Error('Connection failed'));
      mockTwitterClient.testConnection.mockResolvedValue(true);
    ...
    it('should handle Twitter connection failure', async () => {
      ...
      mockElfaClient.testConnection.mockResolvedValue(true);
      mockTwitterClient.testConnection.mockRejectedValue(new Error('Twitter failed'));
