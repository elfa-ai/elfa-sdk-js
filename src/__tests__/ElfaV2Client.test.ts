...
describe('ElfaV2Client', () => {
  ...
  describe('getAccountSmartStats', () => {
    it('should call account smart stats endpoint', async () => {
      ...
      const result = await client.getAccountSmartStats({ username: 'testuser' });

      expect(mockHttpClient.get).toHaveBeenCalledWith('/v2/account/smart-stats?username=testuser');
      expect(result).toEqual(mockResponse);
    });
  ...
  describe('getMentionsByKeywords', () => {
    ...
    it('should throw ValidationError if keywords are missing', async () => {
      await expect(client.getMentionsByKeywords({
        from: 1640995200,
        to: 1641081600
