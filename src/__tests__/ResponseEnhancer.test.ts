...
describe('ResponseEnhancer', () => {
  ...
  describe('enhanceProcessedMentions', () => {
    ...
    it('should return unenhanced data when no Twitter client', async () => {
      ...
      const result = await enhancerWithoutTwitter.enhanceProcessedMentions(mockMentions, {
        includeContent: true
      });
    ...
    it('should handle Twitter API errors gracefully with fallback', async () => {
      mockTwitterClient.getTweets.mockRejectedValue(new Error('Twitter API error'));
    ...
    it('should throw EnhancementError when fallback is disabled', async () => {
      ...
      await expect(enhancer.enhanceProcessedMentions(mockMentions, {
        includeContent: true,
        fallbackToV2: false
    ...
    it('should handle mentions without valid Twitter URLs', async () => {
      ...
      const result = await enhancer.enhanceProcessedMentions(mentionsWithoutTwitterUrls, {
        includeContent: true
      });
  ...
  describe('extractTweetId', () => {
    it('should extract tweet ID from Twitter URL', () => {
      ...
      expect(enhancerInstance.extractTweetId('https://twitter.com/user/status/123456789'))
        .toBe('123456789');
      expect(enhancerInstance.extractTweetId('https://x.com/user/status/987654321'))
        .toBe('987654321');
      expect(enhancerInstance.extractTweetId('https://example.com/post/123'))
        .toBeNull();
    });
  });
