...
describe('TwitterClient', () => {
  ...
  describe('constructor', () => {
    it('should create client with default options', () => {
      ...
      expect(mockHttpClient.setTwitterAuthHeader).toHaveBeenCalledWith('test-bearer-token');
    });
  ...
  describe('getTweets', () => {
    ...
    it('should throw error for too many tweet IDs', async () => {
      const tweetIds = Array(101).fill(0).map((_, i) => i.toString());
    ...
    it('should handle Twitter API errors', async () => {
      const mockResponse = {
        data: [],
        errors: [
          { title: 'Not Found', detail: 'Tweet not found' }
        ]
      };
      mockHttpClient.get.mockResolvedValue(mockResponse);
  ...
  describe('testConnection', () => {
    it('should return true when connection test succeeds', async () => {
      ...
      expect(mockHttpClient.get).toHaveBeenCalledWith('/tweets/by/username/twitter', {
        timeout: 10000
      });
  ...
  describe('getRateLimitInfo', () => {
    it('should call rate limit endpoint', async () => {
      ...
      expect(mockHttpClient.get).toHaveBeenCalledWith('/application/rate_limit_status');
      expect(result).toEqual(mockResponse);
    });
  });
});