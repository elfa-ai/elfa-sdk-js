...
describe("PaginationHelper", () => {
  ...
  describe("iteratePages", () => {
    it("should iterate through all pages", async () => {
      ...
      const pages = [];
      for await (const page of PaginationHelper.iteratePages(mockFetchPage, {
        pageSize: 2,
      })) {
        pages.push(page);
      }
  ...
  describe("iterateCursor", () => {
    it("should iterate through all cursor pages", async () => {
      ...
      const pages = [];
      for await (const page of PaginationHelper.iterateCursor(mockFetchPage, {
        limit: 2,
      })) {
        pages.push(page);
      }
  ...
  describe("collectAllPages", () => {
    it("should collect all items from all pages", async () => {
      ...
      const allItems = await PaginationHelper.collectAllPages(mockFetchPage, {
        pageSize: 2,
      });
    ...
    it("should respect max items limit", async () => {
      ...
      const allItems = await PaginationHelper.collectAllPages(
        mockFetchPage,
        { pageSize: 2 },
        3,
      );
  ...
  describe("collectAllCursor", () => {
    it("should collect all items from cursor pagination", async () => {
      ...
      const allItems = await PaginationHelper.collectAllCursor(mockFetchPage, {
        limit: 2,
      });
    ...
    it("should respect max items limit for cursor pagination", async () => {
      ...
      const allItems = await PaginationHelper.collectAllCursor(
        mockFetchPage,
        { limit: 2 },
        3,
      );
