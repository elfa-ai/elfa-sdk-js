...
export class TwitterClient {
  ...
  public async getTweets(request: TwitterBatchRequest): Promise<TwitterApiResponse<TwitterTweet[]>> {
    ...
    try {
      const response = await this.httpClient.get<TwitterApiResponse<TwitterTweet[]>>(
        `/tweets?${params.toString()}`
      );

      if (response.errors?.length) {
        const errorMessages = response.errors.map(e => e.title).join(', ');
        throw new TwitterApiError(`Twitter API errors: ${errorMessages}`, undefined, response.errors);
      }
      ...
      throw new TwitterApiError(`Failed to fetch tweets: ${error instanceof Error ? error.message : 'Unknown error'}`, undefined, error);
    }
  }

  public async getTweet(tweetId: string, options: {
    tweetFields?: string[];
    userFields?: string[];
    expansions?: string[];
    ...
    try {
      ...
      throw new TwitterApiError(`Failed to fetch tweet: ${error instanceof Error ? error.message : 'Unknown error'}`, undefined, error);
    }
  }

  public async getUser(userId: string, options: {
    userFields?: string[];
    expansions?: string[];
    ...
    try {
      const response = await this.httpClient.get<TwitterApiResponse<TwitterUser>>(
      ...
      throw new TwitterApiError(`Failed to fetch user: ${error instanceof Error ? error.message : 'Unknown error'}`, undefined, error);
    }
  }

  public async getUserByUsername(username: string, options: {
    userFields?: string[];
    expansions?: string[];
  } = {}): Promise<TwitterApiResponse<TwitterUser>> {
    const params = new URLSearchParams();
    ...
    try {
      ...
      throw new TwitterApiError(`Failed to fetch user by username: ${error instanceof Error ? error.message : 'Unknown error'}`, undefined, error);
    }
  }

  public async testConnection(): Promise<boolean> {
    try {
      const response = await this.httpClient.get('/tweets/by/username/twitter', {
        timeout: 10000
      });
      return !!response;
    } catch (error) {
